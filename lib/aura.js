var base = require('./base'),
    ExtManager = require('./aura.extensions.js');
...
function Aura(config) {
  ...
  if (config.debug) { this.use('./ext/debug'); }
  this.use('./ext/mediator');

  config.widgets = (config.widgets || {});
  config.widgets.sources = (config.widgets.sources || { "default" : "widgets" });
  this.use('./ext/widgets');
  this.use('./ext/widgets/options');
...
Aura.prototype.start = function(options) {
  ...
  if (app.started) {
    global.console.error("Aura already started... !");
    ...
    base.util.each(exts, function(i, ext) {
      if (ext && typeof(ext.afterAppStart) === 'function') {
        try {
          ...
          global.console.error("Error on ", (ext.name || ext) , ".afterAppStart callback: (", e.message, ")", e);
        }
      }
    });
  });
  ...
  app.extensions.onFailure(function() {
    global.console.error("Error initializing app...", app.config.name, arguments);
    app.stop();
  });
