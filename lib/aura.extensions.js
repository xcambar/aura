...
ExtManager.prototype.init = function() {
  ...
  (function _init(extDef) {
    if (extDef) {
      ...
      when.call(undefined, initialized).done(function (resolved) {
        initStatus.resolve(resolved);
      });
    }
  })(extensions.shift());
...
function initExtension(extDef) {
  ...
  req.promise.fail(function () {
    dfd.reject();
  });
  req.promise.done(function (ext) {
    ...
    //@TODO XC Moved
    var init = when(Array.isArray(_fn) ? _fn : [_fn]);
